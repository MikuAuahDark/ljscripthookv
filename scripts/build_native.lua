...
local function parseFunction(namespace, line)
	...
	for w in params:gmatch("[^,]+") do
		...
		-- For object types, pointer means it's out parameter
		if contains(objectType, dataType) then
			outParam = isPointer
		-- char* never return type(?)
		elseif dataType == "char" and isPointer then
			outParam = false
		end
		...
		if isPointer then
			...
			elseif contains(objectType, dataType) then
				assert(objectIndex < 16, "increase object limit size!")
				t.index = objectIndex
	...
	if ret == "void" or ret == "Void" then
		...
		-- union.value = ScriptHookV.nativeCall()[0] -- [0] to dereference the pointer
	...
	if #retVals > 0 then
		...
		for i, v in ipairs(retVals) do
			...
			elseif v.pointer then
				if contains(objectType, v.type) then
					-- (ffi.new("<type>", objects[<index>]))
					finalRets[i] = "(ffi.new(\""..v.type.."\", objects["..v.index.."]))"
				elseif v.type == "Vector3" then
					-- (ffi.new("Vector3", vectors[<index>]))
					finalRets[i] = "(ffi.new(\"Vector3\", vectors["..v.index.."]))"
				else
					-- error("FIXME: return_ptr <type>")
					finalRets[i] = "error(\"FIXME: return_ptr "..v.type.."\")"
				end
			else
